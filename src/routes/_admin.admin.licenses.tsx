...
function LicensesPage() {
  ...
  const createMut = useMutation({
    mutationFn: (input: { client_name: string; duration_days: number; notes?: string }) =>
      create({ data: input }),
    onSuccess: () => {
  ...
  const updateMut = useMutation({
    mutationFn: (input: { id: string; status?: "pending" | "active" | "expired" | "revoked" }) =>
      update({ data: input }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["licenses"] });
  ...
  const renewMut = useMutation({
    mutationFn: (input: { id: string; add_days?: number; expires_at?: string }) =>
      renew({ data: input }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["licenses"] });
      setRenewRow(null);
      toast.success("تم تجديد الاشتراك بنفس الكود");
    },
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gradient-primary text-primary-foreground">
              <Plus className="ms-2 h-4 w-4" /> كود جديد
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                سيتم توليد الكود تلقائياً وحفظه بحالة "قيد الانتظار"
              </DialogDescription>
            </DialogHeader>
            <form
              onSubmit={(e) => {
            ...
            >
              <div className="space-y-2">
                <Label htmlFor="cn">اسم العميل</Label>
                <Input id="cn" name="client_name" required maxLength={120} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="dd">مدة الاشتراك (يوم)</Label>
                <Input
                  id="dd"
                  name="duration_days"
                  type="number"
              ...
              <div className="space-y-2">
                <Label htmlFor="nt">ملاحظات (اختياري)</Label>
                <Input id="nt" name="notes" maxLength={500} />
              </div>
              <DialogFooter>
                <Button
                  type="submit"
      ...
      <Card>
        <CardHeader>
          <CardTitle>كل الأكواد</CardTitle>
        </CardHeader>
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        {l.status !== "revoked" && (
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => updateMut.mutate({ id: l.id, status: "revoked" })}
                          >
                        ...
                        {l.status === "revoked" && (
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => updateMut.mutate({ id: l.id, status: "pending" })}
                          >
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => {
                            if (confirm("تأكيد الحذف؟")) deleteMut.mutate(l.id);
                          }}
                        >
      ...
      <Dialog open={!!renewRow} onOpenChange={(value) => !value && setRenewRow(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>تجديد الكود</DialogTitle>
            <DialogDescription>
              سيبقى الكود والجهاز والبيانات كما هي، وسيتم تحديث تاريخ الانتهاء فقط.
            </DialogDescription>
          </DialogHeader>
          {renewRow && (
            <form
              ...
              onSubmit={(event) => {
                ...
                if (renewMode === "days")
                  renewMut.mutate({ id: renewRow.id, add_days: Number(form.get("days") || 30) });
                else
                  renewMut.mutate({
                    id: renewRow.id,
                    expires_at: new Date(`${String(form.get("date"))}T23:59:59.999Z`).toISOString(),
                  });
              }}
            >
              <div className="grid grid-cols-2 gap-2">
                <Button
                  type="button"
                  variant={renewMode === "days" ? "default" : "outline"}
                  onClick={() => setRenewMode("days")}
                >
                  إضافة أيام
                </Button>
                <Button
                  type="button"
                  variant={renewMode === "date" ? "default" : "outline"}
                  onClick={() => setRenewMode("date")}
                >
                  تاريخ محدد
                </Button>
              </div>
              {renewMode === "days" ? (
                <div className="space-y-2">
                  <Label htmlFor="renew-days">عدد الأيام</Label>
                  <Input
                    id="renew-days"
                    name="days"
                    type="number"
                ...
                <div className="space-y-2">
                  <Label htmlFor="renew-date">تاريخ الانتهاء الجديد</Label>
                  <Input
                    id="renew-date"
                    name="date"
                    type="date"
              ...
              <DialogFooter>
                <Button type="submit" disabled={renewMut.isPending}>
                  {renewMut.isPending && <Loader2 className="ms-2 h-4 w-4 animate-spin" />}تجديد
                </Button>
              </DialogFooter>
            </form>
          )}
