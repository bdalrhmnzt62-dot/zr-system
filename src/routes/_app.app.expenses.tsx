  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-extrabold">المصروفات</h2>
          <p className="text-sm text-muted-foreground">تسجيل مصروفات الورشة</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gradient-primary text-primary-foreground">
              <Plus className="ms-2 h-4 w-4" /> مصروف جديد
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <form
              onSubmit={(e) => {
                ...
                createMut.mutate(Object.fromEntries(f));
              }}
              className="space-y-3"
            >
              ...
              <DialogFooter>
                ...
                >
                  {createMut.isPending && <Loader2 className="ms-2 h-4 w-4 animate-spin" />} حفظ
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>كل المصروفات ({rows.length})</CardTitle>
        </CardHeader>
        <CardContent className="overflow-x-auto">
          {rows.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">لا توجد مصروفات.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
      ...
      <Dialog open={!!editRow} onOpenChange={(o) => !o && setEditRow(null)}>
        <DialogContent>
          ...
          {editRow && (
            ...
            >
              <div className="space-y-2">
                <Label htmlFor="title_e">العنوان</Label>
                <Input
                  id="title_e"
                  name="title"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  <Label htmlFor="category_e">التصنيف</Label>
                  <Input
                    id="category_e"
                    name="category"
                    maxLength={50}
                ...
                <div className="space-y-2">
                  <Label htmlFor="amount_e">المبلغ (ج.م)</Label>
                  <Input
                    id="amount_e"
                    name="amount"
                    type="number"
              ...
              <div className="space-y-2">
                <Label htmlFor="expense_date_e">التاريخ</Label>
                <Input
                  id="expense_date_e"
                  name="expense_date"
                  type="date"
                  required
                  defaultValue={editRow.expense_date}
                  dir="ltr"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="notes_e">ملاحظات</Label>
                <Textarea
                  id="notes_e"
                  name="notes"
                  maxLength={1000}
