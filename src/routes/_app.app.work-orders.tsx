...
function OrderForm({
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <Label htmlFor="total_amount">المبلغ المتوقع (ج.م)</Label>
        <Input
          id="total_amount"
          name="total_amount"
          type="number"
          step="0.01"
          min="0"
          defaultValue={initial?.total_amount ?? "0"}
          dir="ltr"
        />
      </div>
      <DialogFooter>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-extrabold">أوامر الشغل</h2>
          <p className="text-sm text-muted-foreground">متابعة الأعمال الجارية في الورشة</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gradient-primary text-primary-foreground">
              <Plus className="ms-2 h-4 w-4" /> أمر شغل جديد
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>أمر شغل جديد</DialogTitle>
            </DialogHeader>
            <OrderForm
              customers={customers}
      ...
      <Card>
        <CardHeader>
          <CardTitle>كل الأوامر ({orders.length})</CardTitle>
        </CardHeader>
        <CardContent className="overflow-x-auto">
          {isLoading ? (
            ...
            <p className="py-8 text-center text-sm text-muted-foreground">لا توجد أوامر شغل.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {orders.map((o: any) => (
                  <TableRow key={o.id}>
                    ...
                    <TableCell className="font-mono text-sm" dir="ltr">
                      {Number(o.total_amount || 0).toLocaleString("en-US")}
                    </TableCell>
                    ...
                    <TableCell className="text-end">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => {
                            if (confirm("حذف؟")) delMut.mutate(o.id);
                          }}
                          title="حذف"
                        >
      ...
      <Dialog open={!!editRow} onOpenChange={(o) => !o && setEditRow(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>تعديل أمر الشغل</DialogTitle>
          </DialogHeader>
          {editRow && (
            <OrderForm
              initial={editRow}
